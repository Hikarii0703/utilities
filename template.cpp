...
// fur Each
#define trav1(_a_, _x_) for (auto &_a_ : _x_)
#define trav2(_a_, _b_, _x_) for (auto &[_a_, _b_] : _x_)
#define trav3(_a_, _b_, _c_, _x_) for (auto &[_a_, _b_, _c_] : _x_)

#define NumArgsT(_1, _2, _3, _4, _5, furTFunc, ...) furTFunc
#define NumArgs(_1, _2, _3, _4, furFunc, ...) furFunc
#define NumArgsE(_1, _2, _3, _4, travFunc, ...) travFunc
...
#define trav(...) NumArgsE(__VA_ARGS__, trav3, trav2, trav1) (__VA_ARGS__)
...
class Reader { private: bool _M_ok_ = true; public: explicit operator bool() { return _M_ok_; } template <typename T> Reader &operator, (T &_t_) { if (!(std::cin >> _t_)) _M_ok_ = false; return *this; } };
#define re Reader(),
...
template <char _sep_, char _end_> class Writer { private: bool _M_ok_ = true; bool _space_; void wrCh(char _c_) { if (_c_ != 0) std::cout << _c_; } public: Writer() : _space_(false) {} ~Writer() { wrCh(_end_); } explicit operator bool() { return _M_ok_; } template <typename T> Writer &operator, (const T &_t_) { (_space_) ? (wrCh(_sep_)) : (void(_space_ = true)); if (!(std::cout << _t_)) _M_ok_ = false; return *this; } };
